...
describe "cj4b4 helps me build both erb files which act as Rails templates" do
  ...
  it "Should run the sql script a1_us_stk_new.sql" do
    `which sqt`.should == "/pt/w/b4/cj4b4/bin/sqt\n"
    # The script should have an exit so it will not hang:
    `grep exit a1_us_stk_new.sql`.should match /^exit\n/
    time0 = Time.now
    sql_output = `sqt @a1_us_stk_new.sql`
    ...
    sql_output.should match /a1_us_stk_new.sql/
    sql_output.should match /^Disconnected from Oracle Database 11g /
    # I should see a recent spool file:
    (Time.now - File.ctime("/tmp/_a1_us_stk_new_spool.html.erb")).should < 9
    # Do a small edit:
    `grep -v 'rows selected' /tmp/_a1_us_stk_new_spool.html.erb > /tmp/a1_us_stk_new.html`
    (Time.now - File.ctime("/tmp/a1_us_stk_new.html")).should < 2
  end
##

  # I Use Nokogiri to massage the HTML in a1_us_stk_new.html and redirect it into a partial holding a table-element.
  # The partial is here:
  # ./a1_us_stk_new/_a1_us_stk_new_spool.html.erb
  # The partial is rendered in this file: 
  # ./a1_us_stk_new/index.html.slim

  it "Uses Nokogiri to massage a1_us_stk_new.html into a partial holding a table-element." do
    myf = File.open("/tmp/a1_us_stk_new.html")
    html_doc = Nokogiri::HTML(myf)
    myf.close
    table_elem = html_doc.search("table.table_a1_us_stk_new").first
    ...
    unless table_elem.nil?
      ...
      td_elems.each {|elm|
        ...
        if ei_h =~ /(tkr: )([A-Z]+)/
          ...
          elm.inner_html = "<a target='g' href='http://www.google.com/finance?q=#{mytkr}'>#{mytkr}</a>"
        end
      }
    ...
    partial_fn = "./a1_us_stk_new/_a1_us_stk_new_spool.html.erb"
