...
if [ -e "/tmp/cron_us_stk_new_is_busy.txt" ]; then
   echo The /tmp/cron_us_stk_new_is_busy.txt exists.
   echo If the /tmp/cron_us_stk_new_is_busy.txt exists, I should exit.
   exit
else
  date > /tmp/cron_us_stk_new_is_busy.txt
  ...
  cd /pt/w/b4/b4/
  git add .
  git commit -a -v -m cron_us_stk_new.bash-commit
  ...
  rm -f  /tmp/cron_us_stk_new_is_busy.txt
fi
